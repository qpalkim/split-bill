...
- **Task 015: 배분 방식 지정 로직 구현 (F003)**
  ...
  #### 테스트 체크리스트
  - [ ] Playwright MCP: 10,000원 / 부담자 3명 균등 배분 시 각 3,333원, 나머지 1원이 결제자에게 가산되는지 확인
  - [ ] Playwright MCP: 나누어떨어지는 금액(9,000원 / 3명)에서 나머지 가산이 발생하지 않는지 확인
  - [ ] Playwright MCP: 부담자 미선택 상태에서 저장 버튼이 차단되는지 확인
  - [ ] Playwright MCP: 항목별 배분에서 합계가 총액보다 적을 때/많을 때 각각 저장이 차단되고 차액이 표시되는지 확인
  - [ ] Playwright MCP: 항목별 배분 합계를 총액과 일치시키면 저장이 성공하는지 확인
  - [ ] Playwright MCP: 균등 → 항목별 전환 시 입력 상태가 규칙대로 초기화/이관되는지 확인
  - [ ] 엣지 케이스: 부담자 1명(전액 부담), 금액 1원, 참여자 20명 대량 배분 검증
...
- **Task 017: 정산 자동 계산 엔진 구현 (F005)**
  ...
  #### 테스트 체크리스트
  - [ ] Playwright MCP: 참여자 3명·지출 3건 시나리오를 UI로 입력해 송금 내역이 수기 계산 결과와 일치하는지 확인
  - [ ] Playwright MCP: 모든 송금 금액의 합이 채권자 총 잔액과 일치하는지 확인
  - [ ] Playwright MCP: 송금 건수가 (참여자 수 − 1) 이하인지 확인
  - [ ] Playwright MCP: 1인이 전부 결제한 단순 케이스에서 송금이 (인원 − 1)건으로 나오는지 확인
  - [ ] Playwright MCP: 균등 배분 나머지 가산이 최종 잔액에 정확히 반영되는지 확인
  - [ ] 엣지 케이스: 전원 잔액 0(정산 불필요), 참여자 2명 최소 케이스, 참여자 15명 대량 케이스 검증
...
- **Task 018: 정산 결과 페이지 실데이터 연동 (F005)**
  ...
  #### 테스트 체크리스트
  - [ ] Playwright MCP: `/result` 직접 진입 시 데이터 부족이면 리다이렉트되는지 확인
  - [ ] Playwright MCP: 지출 수정 후 정산 결과가 즉시 재계산되어 표시되는지 확인
  - [ ] Playwright MCP: 요약 테이블의 부담 합계가 총 지출액과 일치하는지 확인
...
- **Task 018-1: 핵심 기능 통합 테스트**
  ...
  #### 테스트 체크리스트
  - [ ] Playwright MCP: 총무 시나리오 전체 플로우를 처음부터 끝까지 무오류 통과
  - [ ] Playwright MCP: 플로우 중간에 새로고침해도 이어서 진행 가능한지 확인
  - [ ] Playwright MCP: 뒤로가기/앞으로가기 시 상태와 화면이 일치하는지 확인
  - [ ] Playwright MCP: `browser_console_messages`로 콘솔 에러·경고 0건 확인
  - [ ] Playwright MCP: 모바일 뷰포트(390×844)에서 동일 플로우 재검증
...
- **Task 019: 정산 결과 이미지 다운로드 구현 (F006)**
  ...
  #### 테스트 체크리스트
  - [ ] Playwright MCP: "이미지로 다운로드" 클릭 시 PNG 다운로드가 트리거되는지 확인
  - [ ] Playwright MCP: 캡처 영역에 버튼 등 불필요 요소가 포함되지 않는지 스크린샷 대조
  - [ ] Playwright MCP: 참여자 15명 대량 데이터에서도 이미지 생성이 완료되는지 확인
  - [ ] Playwright MCP: 변환 실패 시 에러 토스트가 표시되고 앱이 정상 동작하는지 확인
...
- **Task 021: 세션 초기화 및 데이터 관리 UX**
  ...
  #### 테스트 체크리스트
  - [ ] Playwright MCP: 세션 초기화 후 localStorage 데이터가 제거되고 첫 화면으로 이동하는지 확인
  - [ ] Playwright MCP: 초기화 다이얼로그에서 취소 시 데이터가 보존되는지 확인
  - [ ] Playwright MCP: 기존 세션 복원 배너가 정확한 모임 이름·일시를 표시하는지 확인
